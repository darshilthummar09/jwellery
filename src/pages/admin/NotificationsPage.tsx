...
export function AdminNotificationsPage() {
  ...
  const basePath = role === 'super-admin' ? '/dashboard/super-admin' : '/dashboard/admin';

  const myNotifications = notifications.filter((n) => n.role === 'admin');

  const handleNotificationClick = (n: AppNotification) => {
    markNotificationRead(n.id);
    if (n.type === 'chat' && n.threadId) {
      navigate(`${basePath}/chats?thread=${encodeURIComponent(n.threadId)}`);
    } else if (n.type === 'order' && n.orderId) {
      navigate(`${basePath}/orders?id=${n.orderId}`);
    }
  };

  return (
    <PageContainer>
      <PageTitle title="Notifications" subtitle="Order updates, customer messages, and system alerts." className="mb-8" />
      <NotificationsView
