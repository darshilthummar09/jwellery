...
export function CustomerDashboard() {
  ...
  const activeOrders = myProjects.filter(p => p.status !== 'Completed' && p.status !== 'Rejected');
  const readyOrders = myProjects.filter(p => p.status === 'Completed');
        ...
        <div className="space-y-5">
          {myProjects.length === 0 ? (
            ...
            myProjects.map((project) => (
              <div key={project.id} className="p-4 rounded-xl border border-slate-100 hover:border-emerald-200 hover:bg-emerald-50/30 transition-all">
                <div className="flex items-start justify-between mb-3">
                  <div>
                    <p className="text-sm font-semibold text-slate-800">{project.name}</p>
                    <p className="text-xs text-slate-400 mt-0.5">Due Date: {project.due}</p>
                  </div>
                  <span className={`px-2.5 py-1 rounded-full text-xs font-medium ${
                    project.status === 'Completed' ? 'bg-emerald-100 text-emerald-700' :
                    project.status === 'Pending Approval' ? 'bg-amber-100 text-amber-700' :
                ...
                <div className="w-full bg-slate-100 rounded-full h-1.5">
                  <div
                    className="bg-emerald-505 h-1.5 rounded-full transition-all"
                    style={{ width: project.progress }}
                  />
                </div>
                <div className="flex justify-between mt-1">
                  <span className="text-[10px] text-slate-400">Progress</span>
                  <span className="text-[10px] font-medium text-emerald-600">{project.progress}</span>
                </div>
              </div>
            ))
          )}
